...
const auto expected = gap * sample;
...
int main() {
...

  {
    ...
    printf("sample every 100 ticks, report the delta between the actual and expected time\n");
    ...
    auto tickCount = unifex::sync_wait(
        ...
        | filter_each([&](auto tick){return (tick - first) % expected == 0ms;})
        // include actual time of event
        | then_each([](auto tick){return std::make_tuple(std::chrono::steady_clock::now(), tick);})
        // log ticks that arrive and accumulate a count of the ticks that arrived
        | reduce_each(0, [&](int count, const auto& itemSender) {
          return itemSender
          | unifex::then([&, count](auto tpl){
            ...
            printf("delta is %.4fms at sample %3d, %3.4fms after initial tick\n", delta, count, millis);
            return count + 1;
          });
        })
