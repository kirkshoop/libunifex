...
UNIFEX_CONCEPT
same_as = UNIFEX_IS_SAME(A, B) && UNIFEX_IS_SAME(B, A);
...
template <typename From, typename To>
UNIFEX_CONCEPT
convertible_to = std::is_convertible_v<std::add_rvalue_reference_t<From>, To> &&
...
UNIFEX_CONCEPT
derived_from =
    UNIFEX_IS_BASE_OF(U, T) && UNIFEX_FRAGMENT(unifex::_derived_from, T, U);
...
template <typename T, typename U>
UNIFEX_CONCEPT
assignable_from = std::is_lvalue_reference_v<T> &&
...
template <typename T>
UNIFEX_CONCEPT
swappable = UNIFEX_FRAGMENT(unifex::_swappable, T);
...
template <typename T>
UNIFEX_CONCEPT
equality_comparable = detail::weakly_equality_comparable_with_<T, T>;
...
UNIFEX_CONCEPT
totally_ordered =
    equality_comparable<T> && UNIFEX_FRAGMENT(unifex::_totally_ordered, T);
...
template <typename T, typename U>
UNIFEX_CONCEPT
totally_ordered_with = totally_ordered<T> && totally_ordered<U> &&
...
UNIFEX_CONCEPT
constructible_from = destructible<T> && UNIFEX_IS_CONSTRUCTIBLE(T, Args...);

template <typename T>
UNIFEX_CONCEPT
default_constructible = constructible_from<T>;

template <typename T>
UNIFEX_CONCEPT
move_constructible = constructible_from<T, T> && convertible_to<T, T>;
...
UNIFEX_CONCEPT_FRAGMENT(
    ...
    requires()(0) &&                        //
        ...
        constructible_from<T, T const> &&   //
        convertible_to<T&, T> &&            //
        convertible_to<T const&, T> && convertible_to<T const, T>);
template <typename T>
UNIFEX_CONCEPT
copy_constructible =
    move_constructible<T> && UNIFEX_FRAGMENT(unifex::_copy_constructible, T);
...
template <typename T>
UNIFEX_CONCEPT
semiregular = copyable<T> && default_constructible<T>;
