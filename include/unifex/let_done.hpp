...
public:
  ...
  auto start() & noexcept { return unifex::start(sourceOp_.get()); }
...
typename _rcvr<Source, Done, Receiver, Tag, SenderFactory...>::type::
  ...
  if constexpr (
    ...
    op->startedOp_ = 0 - 1;
    return {result_or_null_tail_sender(unifex::start, op->finalOp_.get())};
  } else {
    UNIFEX_TRY {
      op->startedOp_ = 0;
      unifex::deactivate_union_member(op->sourceOp_);
    ...
    UNIFEX_CATCH(...) {
      return {result_or_null_tail_sender(
          unifex::set_error, consume_receiver(), std::current_exception())};
    }
  }
}
