...
class _op<Sender, Receiver>::type {
  struct wrapped_receiver {
    ...
    template(typename... Values)              //
      ...
      auto rec = std::move(get_receiver());
      destroy(std::move(allocator));
      return unifex::set_value(std::move(rec), (Values &&) values...);
    }
    ...
    template(typename Error)           //
      ...
      auto rec = std::move(get_receiver());
      destroy(std::move(allocator));
      return unifex::set_error(std::move(rec), (Error &&) error);
    }
    ...
    auto set_done() && noexcept {
      ...
      auto rec = std::move(get_receiver());
      destroy(std::move(allocator));
      return unifex::set_done(std::move(rec));
    }
    ...
    template(typename CPO)                                          //
        ...
        friend auto tag_invoke(CPO cpo, const wrapped_receiver& r)  //
...
public:
  ...
  auto start() & noexcept { return unifex::start(inner_); }
...
inline const struct _fn {
  ...
  template(typename Sender, typename Receiver)      //
      ...
      auto
      operator()(Sender&& sender, Receiver&& receiver) const {
    return ((Sender &&) sender).submit((Receiver &&) receiver);
  }
  template <typename Sender, typename Receiver>
  using result_t = variant_tail_sender<
      ...
      callable_result_t<
          tag_t<unifex::start>,
          _submit::operation<Sender, Receiver>&>>;
  ...
  template(typename Sender, typename Receiver)      //
    ...
    switch (blocking(sender)) {
      ...
      case blocking_kind::always_inline: {
        ...
        return result_or_null_tail_sender(unifex::start, op);
        break;
      }
      default: {
