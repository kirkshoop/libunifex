...
//
// when the tick_receiver set_value() is called, start() is called on a factoryOp 
// obtained from the SenderFactory
//
...
//
// when the factory_receiver set_value() is called, start() is called on a new tickOp 
//
// fixed overhead and fully-typed and supports lock-step
//
// clean separation of scopes for nested expressions on items
//
// errors and cancellation from the expression for an item are proxied to the sequence
// scope which stops the whole sequence by emiting them to the sequence receiver.
//
...
inline constexpr struct interval_fn {
  template<typename Clock, typename FinalReceiver, typename SenderFactory, typename Receiver>
  struct factory_op {
    struct type;
  };
...
template<typename Clock, typename FinalReceiver, typename SenderFactory, typename Receiver>
struct interval_fn::factory_op<Clock, FinalReceiver, SenderFactory, Receiver>::type {
  ...
  friend void tag_invoke(unifex::tag_t<unifex::start>, type& self) noexcept {
    try {
      unifex::set_value(std::move(self.receiver_), self.expected_);
...
struct interval_fn::op<Clock, Receiver, SenderFactory>::factory_sender {
  ...
  using factory_op = typename interval_fn::factory_op<Clock, Receiver, SenderFactory, FactoryReceiver>::type;
  using time_point = typename Clock::time_point;
  using duration = typename Clock::duration;
  ...
  template<typename Receiver2>
  friend auto tag_invoke(unifex::tag_t<unifex::connect>, const factory_sender& self, Receiver2&& receiver)
    -> factory_op<Receiver2> {
...
struct interval_fn::op<Clock, Receiver, SenderFactory>::factory_receiver {
  ...
  friend void tag_invoke(unifex::tag_t<unifex::set_value>, factory_receiver&& self, VN&&...) {
    static_assert(sizeof...(VN) == 0, "interval expects the sender factory to return a sender of void");
    auto op = self.op_;
    op->tick_ += op->gap_;
  ...
  friend void tag_invoke(unifex::tag_t<unifex::set_error>, factory_receiver&& self, Error&& error) noexcept {
    auto op = self.op_;
    // end sequence with the error
    unifex::set_error(std::move(op->receiver_), (Error&&) error);
  }
  friend void tag_invoke(unifex::tag_t<unifex::set_done>, factory_receiver&& self) noexcept {
    auto op = self.op_;
...
struct interval_fn::op<Clock, Receiver, SenderFactory>::tick_receiver {
  ...
  friend void tag_invoke(unifex::tag_t<unifex::set_done>, tick_receiver&& self) noexcept {
    // end sequence. This sequence can only be ended by error or cancellation. 
    // composition is helped by treating cancellation as successful completion
    unifex::set_value(std::move(self.op_->receiver_));
  }
  template(typename CPO, typename R)
      (requires unifex::is_receiver_query_cpo_v<CPO> AND
...
struct interval_fn::op<Clock, Receiver, SenderFactory>::type {
  ...
  type(
    ...
    factoryOp_(unifex::connect(
      sf_(factory_sender{this, tick_}), 
      factory_receiver{this}))
  {}
...
struct interval_fn::sender<Clock>::type {
  ...
  friend auto tag_invoke(unifex::tag_t<unifex::sequence_connect>, const type& self, Receiver&& receiver, SenderFactory&& sf)
    -> typename op<Clock, Receiver, SenderFactory>::type {
    auto scheduler = unifex::get_scheduler(receiver);
    return {self.reference_, self.gap_, (Receiver&&)receiver, (SenderFactory&&)sf, std::move(scheduler)};
  }
};
} // namespace _inrvl_cpo
