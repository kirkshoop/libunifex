...
inline constexpr struct then_fn {
  ...
  struct state;
  template<typename Predecessor, typename SuccessorReceiver, typename SenderFactory, typename Transform>
  struct op {
    struct type;
  };
  ...
  constexpr auto operator()(Transform&& trns) const
    noexcept(is_nothrow_callable_v<
        tag_t<bind_back>, then_fn, Transform>)
...
template<typename Predecessor, typename SuccessorReceiver, typename SenderFactory, typename Transform>
struct then_fn::op<Predecessor, SuccessorReceiver, SenderFactory, Transform>::type {
  ...
  using pred_op_t = sequence_connect_result_t<Predecessor, SuccessorReceiver, then_each>;
  state_t state_;
  pred_op_t predOp_;

  template<typename Predecessor2, typename SuccessorReceiver2, typename SenderFactory2, typename Transform2>
...
struct then_fn::sender<Predecessor, Transform>::type {
  ...
  using result = type_list<
    typename detail::result_overload<callable_result_t<Transform, Args...>>::type>;
  ...
  tag_invoke(unifex::tag_t<unifex::sequence_connect>, const type& self, Receiver&& receiver, SenderFactory&& sf) {
    return {self.predecessor_, (Receiver&&)receiver, (SenderFactory&&)sf, self.trns_};
  }
};
} // namespace _thn_cpo
