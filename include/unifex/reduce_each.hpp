...
inline constexpr struct reduce_fn {
  template<typename SuccessorReceiver, typename Value, typename AccumulatorFactory>
  struct succ_rcvr {
    struct type;
  };
  template<typename SuccessorReceiver, typename Value, typename AccumulatorFactory>
  struct state;
  template<typename Predecessor, typename SuccessorReceiver, typename Value, typename AccumulatorFactory>
  struct op {
    struct type;
  };
...
template<typename Predecessor, typename SuccessorReceiver, typename Value, typename AccumulatorFactory>
struct reduce_fn::op<Predecessor, SuccessorReceiver, Value, AccumulatorFactory>::type {
  using succ_rcvr_t = typename succ_rcvr<SuccessorReceiver, Value, AccumulatorFactory>::type;
  ...
  struct update {
    ...
    auto operator()(ItemSender&& itemSender) {
        return state_->af_(std::as_const(state_->v_), (ItemSender&&)itemSender) | then([state_ = this->state_](Value newValue){
  ...
  template<typename Predecessor2, typename SuccessorReceiver2, typename Value2, typename AccumulatorFactory2>
  type(Predecessor2&& predecessor,
    SuccessorReceiver2&& successorReceiver,
    Value2&& v,
    AccumulatorFactory2&& af) 
    : state_({(SuccessorReceiver2&&)successorReceiver, (Value2&&)v, (AccumulatorFactory2&&)af})
