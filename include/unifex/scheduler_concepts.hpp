...
public:
  ...
  template(typename Scheduler)                //
      (requires                               //
       (!_with_tag_invoke<Scheduler>) AND     //
...
concept scheduler =  //
    ...
    equality_comparable<remove_cvref_t<S>> && requires(S&& s) {
  schedule((S &&) s);
};
#else
template <typename S>
UNIFEX_CONCEPT_FRAGMENT(_scheduler, requires(S&& s)(schedule((S &&) s)));
template <typename S>
UNIFEX_CONCEPT
scheduler =  //
    copy_constructible<remove_cvref_t<S>> &&
...
namespace _get_scheduler {
  inline const struct _fn {
    template(typename SchedulerProvider)                //
        (requires                                       //
         tag_invocable<_fn, const SchedulerProvider&>)  //
        auto
...
// Define the scheduler concept without the macros for better diagnostics
#if UNIFEX_CXX_CONCEPTS
template <typename SProv>
concept scheduler_provider = requires(SProv&& sp) {
  get_scheduler((SProv &&) sp);
};
#else
template <typename SProv>
UNIFEX_CONCEPT_FRAGMENT(
    _scheduler_provider, requires(SProv&& sp)(get_scheduler((SProv &&) sp)));
template <typename SProv>
UNIFEX_CONCEPT
scheduler_provider =  //
    UNIFEX_FRAGMENT(unifex::_scheduler_provider, SProv);
#endif
...
namespace _schedule {
  ...
  inline constexpr sender _fn::operator()() const noexcept { return {}; }
}  // namespace _schedule
...
public:
  ...
  template(typename TimeScheduler, typename Duration)   //
      ...
      constexpr auto
      operator()(TimeScheduler&& s, Duration&& d) const  //
      noexcept(noexcept(static_cast<TimeScheduler&&>(s).schedule_after(
          (Duration &&) d))) -> _result_t<TimeScheduler, Duration> {
...
using _schedule_after::schedule_after_result_t;
...
public:
  ...
  template(typename TimeScheduler, typename TimePoint)   //
      ...
      operator()(TimeScheduler&& s, TimePoint&& tp) const  //
      noexcept(noexcept(
          static_cast<TimeScheduler&&>(s).schedule_at((TimePoint &&) tp)))  //
      -> _result_t<TimeScheduler, TimePoint> {
...
using _schedule_at::schedule_at_result_t;
...
inline constexpr struct _scheduler {
  auto schedule() const noexcept { return unifex::schedule(); }
  template <typename Duration>
