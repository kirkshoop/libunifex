...
using _member_sequence_connect_result_t =
    decltype((UNIFEX_DECLVAL(Sender &&))
                 .sequence_connect(
                     UNIFEX_DECLVAL(Receiver &&),
                     UNIFEX_DECLVAL(SenderFactory&&)));
...
UNIFEX_CONCEPT_FRAGMENT(
    ...
    requires()  //
    (typename(
        _member_sequence_connect_result_t<Sender, Receiver, SenderFactory>)));
template <typename Sender, typename Receiver, typename SenderFactory>
UNIFEX_CONCEPT
_with_member_sequence_connect =  //
    sender<Sender> &&
    UNIFEX_FRAGMENT(
        _sequence_connect::_has_member_sequence_connect_,
        Sender,
        Receiver,
        SenderFactory);
...
private:
  ...
  static auto _select() {
    ...
    } else if constexpr (_with_member_sequence_connect<
                             Sender,
                             Receiver,
                             SenderFactory>) {
  ...
  template <typename Sender, typename Receiver, typename SenderFactory>
  using _result_t =
      typename decltype(_fn::_select<Sender, Receiver, SenderFactory>())::
          template apply<Sender, Receiver, SenderFactory>;

public:
  template(typename Sender, typename Receiver, typename SenderFactory)(
      requires receiver<Receiver> AND
          _with_tag_invoke<Sender, Receiver, SenderFactory>) auto
  operator()(Sender&& s, Receiver&& r, SenderFactory&& sf) const
      noexcept(is_nothrow_tag_invocable_v<_fn, Sender, Receiver, SenderFactory>)
          -> _result_t<Sender, Receiver, SenderFactory> {
    return unifex::tag_invoke(
        _fn{}, (Sender &&) s, (Receiver &&) r, (SenderFactory &&) sf);
  }
  template(typename Sender, typename Receiver, typename SenderFactory)(
      requires receiver<Receiver> AND(
          !_with_tag_invoke<Sender, Receiver, SenderFactory>) AND
          _with_member_sequence_connect<Sender, Receiver, SenderFactory>) auto
  operator()(Sender&& s, Receiver&& r, SenderFactory&& sf) const noexcept(
      noexcept(((Sender &&) s)
                   .sequence_connect((Receiver &&) r, (SenderFactory &&) sf)))
      -> _result_t<Sender, Receiver, SenderFactory> {
    return ((Sender &&) s)
        .sequence_connect((Receiver &&) r, (SenderFactory &&) sf);
  }
...
template <typename Sender, typename Receiver, typename SenderFactory>
using sequence_connect_result_t = decltype(sequence_connect(
    UNIFEX_DECLVAL(Sender),
    UNIFEX_DECLVAL(Receiver),
    UNIFEX_DECLVAL(SenderFactory)));
